...
function [hArray, wArray, newtonSuccess] = getQuad(self, freq, numPts, QuadParams)
% the two outputs are the complex quadrature nodes and weights respectively
    ...
    if isinf(self.length)
        ...
        if strcmp(QuadParams.infQuadRule,'laguerre')
            % get Gauss Laguerre weights and nodes
            [gaussNodes, gaussWeights] = gausLagHC(numPts);
        elseif strcmp(QuadParams.infQuadRule,'legendre')
            ...
            machinePrecisionLength = ...
                abs(log(QuadParams.maxSPintegrandVal*...
    ...
    if max(gaussNodes) > freq*max(self.coarseParam)
        ...
        if QuadParams.mex
            [self.coarseParam, self.coarsePath, success] = ...
                sdPathODEeulerExtendCoarsePathMex(self.coarseParam, ...
                self.coarsePath, self.phaseCoeffs(:), stationaryPoints, ...
                QuadParams.globalStepSize, ...
                int64(QuadParams.maxNumODEsteps), max(gaussNodes)/freq);
        else
            [self.coarseParam, self.coarsePath, success] = ...
                sdPathODEeulerExtendCoarsePath(self.coarseParam, ...
                self.coarsePath, self.phaseCoeffs, stationaryPoints, ...
                QuadParams.globalStepSize, ...
                int64(QuadParams.maxNumODEsteps), max(gaussNodes)/freq);
        end
    ...
    if isinf(self.length) && strcmp(QuadParams.infQuadRule,'laguerre')
