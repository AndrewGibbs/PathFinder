                               ...
                               const emlrtRTEInfo *srcLocation)
{
  ...
  if (newNumel > emxArray->allocatedSize) {
    ...
    newData = emlrtCallocMex((uint32_T)i, sizeof(int64_T));
    if (newData == NULL) {
      emlrtHeapAllocationErrorR2012b(srcLocation, (emlrtCTX)sp);
    }
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(int64_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }
...
/* End of code generation (SDpathODEuler_emxutil.c) */
