...
static emlrtRSInfo emlrtRSI = {
    60,              /* lineNo */
    "SDpathODEuler", /* fcnName */
    "/home/andrew/Dropbox/PathFinderInf/src/contours/SDpathODEuler.m" /* pathName
                                                                       */
};
...
static emlrtBCInfo b_emlrtBCI = {
    ...
    "gCoeffs",       /* aName */
    "SDpathODEuler", /* fName */
    "/home/andrew/Dropbox/PathFinderInf/src/contours/SDpathODEuler.m", /* pName
                                                                        */
    0 /* checkKind */
};
...
static boolean_T SDpathODEuler_anonFcn3(const emlrtStack *sp,
                                        ...
                                        const creal_T h)
{
  ...
  emxInit_creal_T(sp, &r, 1, &s_emlrtRTEI);
  if (value) {
    varargout_1 = true;
  } else {
    st.site = &m_emlrtRSI;
    ...
    if (theta_tmp == 0.0) {
      ...
      theta_L = b_r * (b_r * muDoubleScalarCos(theta_tmp));
      theta_tmp = b_r * (b_r * muDoubleScalarSin(theta_tmp));
    }
    idx = r->size[0];
    r->size[0] = b->size[0];
    emxEnsureCapacity_creal_T(&b_st, r, idx, &s_emlrtRTEI);
    r1 = r->data;
    last = b->size[0];
    ...
    if ((idx < 1) || (idx > valleys->size[0])) {
      emlrtDynamicBoundsCheckR2012b(idx, 1, valleys->size[0], &u_emlrtBCI, &st);
    }
    ...
    theta_diff = muDoubleScalarMax(
        theta_tmp, 3.1415926535897931 / (4.0 * (real_T)valleys->size[0]));
    /*  if theta_diff is less than this optimal value, truncate it there */
    /*      for v = mod(angle(V),2*pi) */
    /*          first check if theta_end is pointing sufficiently close to
     * valley */
    theta_L_tmp = v - theta_diff;
    theta_L = b_mod(theta_L_tmp - 2.2204460492503131E-15);
    theta_tmp = v + theta_diff;
    b_r = b_mod(theta_tmp + 2.2204460492503131E-15);
    guard1 = false;
    if (theta_L < b_r) {
      if ((theta_L < theta) && (theta < b_r)) {
        guard1 = true;
      } else {
        value = false;
      }
    ...
    } else if ((theta_L < theta) || (theta < b_r)) {
    ...
    if (guard1) {
      if (R > 1.0) {
        theta_tmp =
            muDoubleScalarSin((real_T)valleys->size[0] *
                              (theta_tmp + argBeta / (real_T)valleys->size[0]));
        b_r = muDoubleScalarCos(
            (real_T)valleys->size[0] *
            (theta_L_tmp + argBeta / (real_T)valleys->size[0]));
        if ((theta_tmp > b_r) ||
            (muDoubleScalarIsNaN(theta_tmp) && (!muDoubleScalarIsNaN(b_r)))) {
          theta_tmp = b_r;
        }
        if ((R * theta_tmp > Cnr) &&
            (theta_diff <
                             ...
                             const emxArray_real_T *r1)
{
  ...
  emxEnsureCapacity_creal_T(sp, abs_dF_workspace_ddgCoeffs, i, &j_emlrtRTEI);
                   ...
                   int32_T ball_index_size[2], boolean_T *success)
{
  ...
  *success = true;
  ...
  emxInit_creal_T(sp, &abs_dF_workspace_ddgCoeffs, 2, &p_emlrtRTEI);
  if (last == r->size[1]) {
  ...
  do {
    ...
    varargout_1 = SDpathODEuler_anonFcn3(&b_st, SPs, cover_radii, Cnr, argBeta,
                                         valleys, h);
    if (!varargout_1) {
      n++;
      ...
      step_size =
          base_step_size *
          muDoubleScalarMin(1.0 / muDoubleScalarHypot(b_re, b_im), h_re) /
          muDoubleScalarHypot(step_size_tmp_re, step_size_tmp_im);
...
/* End of code generation (SDpathODEuler.c) */
