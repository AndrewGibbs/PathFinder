function [ChebAij, Avals, AvalsCheck] = Aij2(aValley,bValley,Pcoeffs,freq,Npts,degs,range)
...
% mins
    ...
    AvalsCheck = zeros(length(xPts),length(yPts));
    parfor i=1:length(xPts) % ** this line can be parfor **
        fprintf('\n%d',i);
        Avec = zeros(1,length(yPts));
        AvCheck = zeros(1,length(yPts));
        for j = 1:length(yPts)
            [Avec(j), AvCheck(j)] = contourPoly2(aValley,bValley,PcoeffsXY{i,j},freq,Npts);
        end
        Avals(i,:) = Avec;
        AvalsCheck(i,:) = AvCheck;
    end
